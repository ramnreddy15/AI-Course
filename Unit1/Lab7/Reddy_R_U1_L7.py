...
def make_graph(nodes = "rrNodes.txt", node_city = "rrNodeCity.txt", edges = "rrEdges.txt"):
   ...
   for line in temp:
       splits = line.split(" ")
       nodeLoc[splits[0]] = (splits[1], splits[2][:-1],)
   temp.close()
   
   temp = open(node_city)
   for line in temp:
        place = line.index(" ")
        splits = [line[:place],line[place+1:][:-1]]
        nodeToCity[splits[0]] = splits[1]
        cityToNode[splits[1]] = splits[0]
   temp.close()
   ...
   for line in temp:
       splits = line.split(" ")
       splits[1] = splits[1][:-1]
       cost = calc_edge_cost(nodeLoc[splits[0]][0], nodeLoc[splits[0]][1], nodeLoc[splits[1]][0], nodeLoc[splits[1]][1])
       edgeCost[(splits[0],splits[1],)] = cost
       edgeCost[(splits[1],splits[0],)] = cost
       if splits[0] not in neighbors.keys():
           neighbors[splits[0]] = (splits[1],)
       else:
           neighbors[splits[0]] = neighbors[splits[0]] + (splits[1],)
       if splits[1] not in neighbors.keys():
           neighbors[splits[1]] = (splits[0],)
       else:
           neighbors[splits[1]] = neighbors[splits[1]] + (splits[0],)
   temp.close()
...
def draw_all_edges(ROOT, canvas, graph):
   ...
   for n1, n2 in graph[4]:  #graph[4] keys are edge set
      drawLine(canvas, *graph[5][n1], *graph[5][n2], 'white')
   ROOT.update()
...
def bi_bfs(start, goal, graph, col):
   ...
   while len(Qs) != 0 and len(Qe) != 0:
       ...
       e = Qe.pop(0)
       if s == e:
           path1, cost1 = generate_path(s, explored[0], graph)
           path2, cost2 = generate_path(e, explored[1], graph)
           path2 = path2[:-1][::-1]
           fullPath = path1+path2
           print("The number of explored nodes of Bi-BFS",len(explored[0])+len(explored[1]))
           draw_final_path(ROOT, canvas, fullPath, graph)
           return fullPath,cost1+cost2
       elif s in explored[1]:
           path1, cost1 = generate_path(s, explored[0], graph)
           path2, cost2 = generate_path(s, explored[1], graph)
           path2 = path2[:-1][::-1]
           fullPath = path1+path2
           print("The number of explored nodes of Bi-BFS",len(explored[0])+len(explored[1]))
           draw_final_path(ROOT, canvas, fullPath, graph)
           ...
           path1, cost1 = generate_path(e, explored[0], graph)
           path2, cost2 = generate_path(e, explored[1], graph)
           path2 = path2[:-1][::-1]
           fullPath = path1+path2
           print("The number of explored nodes of Bi-BFS",len(explored[0])+len(explored[1]))
       ...
       for a in graph[3][e]:
           if a not in explored[1].keys():
               Qe.append(a)
               explored[1][a] = e
               drawLine(canvas, *graph[5][e], *graph[5][a], col)
       counter += 1
       if counter % 1000 == 0: ROOT.update()
   return None
...
def a_star(start, goal, graph, col, heuristic=dist_heuristic):
    ...
    explored = set()
    explored.add(start)
    heapq.heappush(frontier, (dist_heuristic(start, goal, graph), start,(start,),0))
    while len(frontier) != 0:
        ...
        if s == goal:
            ...
            return path, get_cost(path, graph)
        for a in graph[3][s]:
            if not a in explored:
                cost = calc_edge_cost(graph[0][a][0], graph[0][a][1], graph[0][s][0], graph[0][s][1])
                explored.add(a)
                drawLine(canvas, *graph[5][s], *graph[5][a], col)
                heapq.heappush(frontier, (dist_heuristic(a, goal, graph)+length+cost, a, path+(a,), length+cost))
        counter += 1
        if counter % 1000 == 0: ROOT.update()
    return None
...
def bi_a_star(start, goal, graph, col, heuristic=dist_heuristic):
    ...
    frontierS = []
    heapq.heappush(frontierS, (heuristic(start, goal, graph), start, (start,),0))
    frontierE = []
    heapq.heappush(frontierE, (heuristic(goal, start, graph), goal, (goal,),0))
    explored = [{start:"s"},{goal:"s"}]
    while len(frontierS) != 0 and len(frontierE) != 0:
        ...
        if s is e:
           ...
           path2 = path2[:-1][::-1]
           fullPath = path1+path2
           print("The number of explored nodes of Bi-A-Star",len(explored[0])+len(explored[1]))
           draw_final_path(ROOT, canvas, fullPath, graph)
           return fullPath,cost1+cost2
        elif s in explored[1]:
            path1, cost1 = generate_path(s, explored[0], graph)
        ...
        for a in graph[3][s]:
           if a not in explored[0].keys():
              cost = calc_edge_cost(graph[0][s][0], graph[0][s][1], graph[0][a][0], graph[0][a][1])
              explored[0][a] = s
              drawLine(canvas, *graph[5][s], *graph[5][a], col)
              heapq.heappush(frontierS,(heuristic(a, goal, graph)+ lengthS +cost, a, pathS+(a,), lengthS+cost))
        for a in graph[3][e]:
           if a not in explored[1].keys():
              cost = calc_edge_cost(graph[0][e][0], graph[0][e][1], graph[0][a][0], graph[0][a][1])
              explored[1][a] = e
              drawLine(canvas, *graph[5][e], *graph[5][a], col)
              heapq.heappush(frontierE,(heuristic(a, start, graph)+ lengthE +cost, a, pathE+(a,),lengthE+cost))
        counter += 1
        if counter % 200 == 0: ROOT.update()
    return None
...
def tri_directional(city1, city2, city3, graph, col, heuristic=dist_heuristic):
    ...
    frontierS = []
    heapq.heappush(frontierS, (heuristic(city1, city2, graph), city1, (city1,),0))
    frontierE = []
    heapq.heappush(frontierE, (heuristic(city2, city3, graph), city2, (city2,),0))
    frontierT = []
    heapq.heappush(frontierT, (heuristic(city3, city1, graph), city3, (city3,),0))
    explored = [{city1:"s"},{city2:"s"},{city3:"s"}]
    while len(frontierS) != 0 and len(frontierE) != 0 and len(frontierT) != 0:
        pathS =  heapq.heappop(frontierS)
        s = pathS[1]
        lengthS = pathS[3]
        pathS = pathS[2]
        ...
        if s is e:
           ...
           path2 = path2[:-1][::-1]
           fullPath = path1+path2
           print("The number of explored nodes of Tri-A-Star",len(explored[1])+len(explored[2])+len(explored[0]))
           draw_final_path(ROOT, canvas, fullPath, graph)
           return fullPath,cost1+cost2 
        elif city3 in explored[1] and city1 in explored[2]:
            path1, cost1 = generate_path(city3, explored[1], graph)
            path2, cost2 = generate_path(city1, explored[2], graph)
            path2 = path2[:-1][::-1]
            fullPath = path1+path2+[city1]
            print("The number of explored nodes of Tri-A-Star",len(explored[1])+len(explored[2])+len(explored[0]))
            draw_final_path(ROOT, canvas, fullPath, graph)
            return fullPath,cost1+cost2
        for a in graph[3][s]:
           if a not in explored[0].keys():
              ...
              explored[0][a] = s
              drawLine(canvas, *graph[5][s], *graph[5][a], col)
              heapq.heappush(frontierS,(heuristic(a, city2, graph)+ lengthS +cost, a, pathS+(a,), lengthS+cost))
        for a in graph[3][e]:
           if a not in explored[1].keys():
              cost = calc_edge_cost(graph[0][e][0], graph[0][e][1], graph[0][a][0], graph[0][a][1])
              explored[1][a] = e
              drawLine(canvas, *graph[5][e], *graph[5][a], col)
              heapq.heappush(frontierE,(heuristic(a, city3, graph)+ lengthE +cost, a, pathE+(a,),lengthE+cost))
        for a in graph[3][t]:
           if a not in explored[2].keys():
              cost = calc_edge_cost(graph[0][t][0], graph[0][t][1], graph[0][a][0], graph[0][a][1])
              explored[2][a] = t
              drawLine(canvas, *graph[5][t], *graph[5][a], col)
              heapq.heappush(frontierT,(heuristic(a, city1, graph)+ lengthT +cost, a, pathT+(a,),lengthT+cost))
        counter += 1
        if counter % 200 == 0: ROOT.update()
    return None
