...
def goodBlockPosition(board, i , j, blocks, makerBoard):
    ...
    if(i>0 and j>0 and i<len(board)-1 and j<len(board[0])-1):
        if(board[i-1][j-1]=="#" and board[i-1][j] == "#" and board[i+1][j+1]=="#") or (board[i-1][j]=="#" and board[i-1][j+1] == "#" and board[i+1][j-1]=="#"): 
            return False
    if i>= 3 and j>=3 and i<len(board)-1 and j<len(board[0])-1:
    ...
    if (i<3 and i>0 and board[i-1][j] == "-") or (i>=len(board)-3 and i<len(board)-1 and board[i+1][j] == "-") or (j<3 and j>0 and board[i][j-1] == "-") or (j>=len(board[0])-3 and j<len(board[0])-1 and board[i][j+1] == "-"): return False
...
def createBoard(matches):
    ...
    if rows == cols and len(matches[2:]) == 0 and (4*rows)-4 <= blocks:
        for i in range(rows):
            board[i][0] = "#"
            board[0][i] = "#"
    if rows*cols == blocks:
    ...
    if rows%2==1 and cols%2 == 1 and rows*cols != blocks:
        if blocks%2==1:
            ...
            makerBoard[int(rows/2)][int(cols/2)] = "~"
            makerBoard[int(rows/2)][(int(cols/2))+1] = "~"
            makerBoard[int(rows/2)][int(cols/2)-1] = "~"
            makerBoard[(int(rows/2))+1][int(cols/2)] = "~"
            makerBoard[(int(rows/2))-1][int(cols/2)] = "~"
    for j in range(len(makerBoard[0])):
        for i in range(len(makerBoard)):
...
def genGoodPos(board, blocks):
    goodPositions = []
    if blocks % 2 == 1:
        goodPositions.append((len(board)//2, len(board[0])//2))
    else:
        goodPositions.append(((len(board)//2)+1, len(board[0])//2))
    moves = [(-1,-1),(1,1),(1,-1),(-1,1)]
    for k in range(min(len(board),len(board[0]))):
        for n in range(len(goodPositions)):
            place = goodPositions[random.randint(0,len(goodPositions)-1)]
            type = moves[random.randint(0,3)]
            i = place[0]+type[0]
            j = place[1] + type[1]
            if(i >=3 and i<len(board)-3 and j>= 3 and j<len(board[0])-3 and (i,j) not in goodPositions):
                goodPositions.append((i,j))
    return goodPositions
    
    
def makeGood(board, makerBoard, blockPositions, blocks, badPositions):
    goodStartPositions = genGoodPos(board, blocks)   
    reset = 1
    times = 0
    while(blocks != 0):
        if times>=300:
...
#                             print(l, end=" ")
#                         print()
                    if makerBoard[i][j] == "-" and goodBlockPosition(board, i, j, blocks, makerBoard) and blocks != 0:
                        board[i][j] = "#"
                        board[len(board)-i-1][len(board[0])-j-1] = "#"
                        makerBoard[i][j] = "#"
                        makerBoard[len(makerBoard)-i-1][len(makerBoard[0])-j-1] = "#"  
                        blocks-=2  
                    elif j>0 and makerBoard[i][j-1]=="#" and (makerBoard[i][j] == "~" or makerBoard[i][j] == "-") and i==0 and j==len(board[0])-3 and blocks >= 6:
                        for c in range(3):
                            board[i][j+c] = "#"
                        ...
                        blocks-=6
                    elif i>0 and makerBoard[i-1][j]=="#" and (makerBoard[i][j] == "~" or makerBoard[i][j] == "-") and j==0 and i==len(board)-3 and blocks >= 6:
                        for c in range(3):
                            board[i+c][j] = "#"
                            board[len(board)-i-1-c][len(board[0])-j-1] = "#"
                            makerBoard[i+c][j] = "#"
                            makerBoard[len(makerBoard)-i-1-c][len(makerBoard[0])-j-1] = "#"  
                        blocks-=6
        elif len(goodStartPositions) == 0:
            ...
            for i,j in goodStartPositions:
                generateBadPositions(makerBoard)        
                if makerBoard[i][j] == "-" and goodBlockPosition(board, i, j, blocks, makerBoard):
                    board[i][j] = "#"
                    board[len(board)-i-1][len(board[0])-j-1] = "#"
                    makerBoard[i][j] = "#"
                    makerBoard[len(makerBoard)-i-1][len(makerBoard[0])-j-1] = "#"  
...
def main():
    ...
    for i in args[2:]:
        ...
        if temp != None:
            ...
            matches2.append(temp[0])
    if len(args[:]) == sum(matches) + (1 if len(file)>0 else 0):
