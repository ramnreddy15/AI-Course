...
def utility(turn, tc, state):
   ...
   for i in tc:
       temp = state[i[0]]+state[i[1]]+state[i[2]]
       if (temp == "OOO" and turn == "X") or (temp == "XXX" and turn == "O"):
            return -1
       if (temp == "XXX" and turn == "X") or (temp == "OOO" and turn == "O"):
            return 1 
...
def max_value(state, turn, tc):
   ...
   if terminal_test(state,tc):
        return (utility(get_turn(state),tc,state), state)
   else:
       v = (-2,state)
       for a,s in successors(state,turn):
           temp3 = min_value(s, get_turn(s),tc)
           if(temp3[0] > v[0]):
            v = (temp3[0],s)
       return v
...
def min_value(state, turn, tc):
   # return value and state: (val, state)
   if terminal_test(state,tc):
        return (-1*utility(get_turn(state),tc,state), state)
   else:
       v = (2,state)
       for a,s in successors(state,turn):
           temp3 = max_value(s, get_turn(s),tc)
           if(temp3[0] < v[0]):
               v = (temp3[0],s)
       return v
